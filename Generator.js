...
//test
    ...
    sumRanValues(num, mode, category = "COMMON") {
      const total = this.DATAOBJ[category][mode].reduce((sum, obj) => sum + (obj.hasOwnProperty('ran') ? parseInt(obj.ran) : 0), 0);
...
app.component('commoncard', {
  ...
  template: `
    <grid_v-card :select="select" :character="charaobj[element.chara]">
      ...
      <v-btn height="30" width="30" icon @click.stop="clearImageArr('BASIC_IMAGES', element,index)" style="position: absolute; top: 0; right: 0;">
        <v-icon>mdi-close</v-icon>
        <v-tooltip activator="parent" location="bottom">この設定を消去</v-tooltip>
      </v-btn>
      <v-chip v-show="!element.price" prepend-icon="mdi-chart-pie" color="secondary" variant="flat" style="position: absolute; bottom: 5px; right: 10px;">
        {{element.ran}} ({{sumRanValues(element.ran)}}%)
      </v-chip>
      <v-chip v-show="element.price" prepend-icon="mdi-gift" color="orange" variant="flat" style="position: absolute; bottom: 5px; right: 10px;">
        {{element.price}} - {{element.priceLimit || ""}}
      </v-chip>
      <v-progress-linear :model-value="sumRanValues(element.ran)" color="primary" striped absolute></v-progress-linear>
    </grid_v-card>
  `,
});
...
// リスト表示
app.component('test', {
  props: ['select', 'character','element','index',],
  data() {
    return {
      dialog: false,
      pattern: {
    ...
    <v-dialog v-model="dialog" max-width="800px">
     <template v-slot:activator="{ on }">
      <v-col :cols="Pattern.cols" :sm="Pattern.sm" :md="Pattern.md" :lg="Pattern.lg">
        <v-card :height="Pattern.height" :style="cardStyle" @click="dialog = !dialog;">
